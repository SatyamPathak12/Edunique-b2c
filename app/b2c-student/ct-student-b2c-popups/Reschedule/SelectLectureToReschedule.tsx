...
const SelectLectureToReschedule: React.FC<PopupPropB2CStudent> = ({ isOpen, onClose }) => {
  const [selectedCells, setSelectedCells] = useState<string[]>(['Wed-13']);

  const toggleCell = (day: string, hour: number) => {
    const key = `${day}-${hour}`;
    // Prevent toggling on cells without slot and not selected already
    const hasSlot = mockSchedule[day]?.[hour] || key === 'Wed-13';
    ...
    <StudentB2CBaseModal isOpen={isOpen} onClose={onClose} maxWidth="max-w-4xl">
      <div className="overflow-x-auto px-4 py-6 text-sm min-w-[800px]">
        <div className="flex justify-between items-center w-full">
          <h2 className="text-2xl font-semibold mb-4">Select Lecture to Reschedule</h2>
          <MonthTab />
        </div>
        {/* Header */}
          ...
          >
            ...
            {days.map((day) => {
              ...
              return (
                ...
                >
                  {isSelected ? (
                    <div className="bg-[#ffc79a] text-white w-full h-[50px] flex items-center justify-center rounded-xl px-2 py-1">
                      <FaCheck className="text-xs" />
                    </div>
                  ) : (
                    slots &&
                    slots.length > 0 && (
                      <div className="w-full bg-[#FFC79A1A] border border-[#ffc79a] rounded-xl px-2 py-1 text-left">
      ...
      <div className="flex justify-center items-center gap-4 pt-2 pb-3">
        <button onClick={onClose} className="max-w-32 font-medium w-full rounded-full text-[#ff3366] bg-[#ff33661a] py-2.5">Cancel</button>
        <button onClick={onClose} className="max-w-32 font-medium w-full rounded-full text-white bg-[#3366ff] py-2.5">Continue</button>
      </div>
    </StudentB2CBaseModal>
  );
};

export default SelectLectureToReschedule;
