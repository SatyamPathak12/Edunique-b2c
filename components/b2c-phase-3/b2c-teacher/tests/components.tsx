...
const ActionsMenu: React.FC<{ type: TestType, testType?: "BW Test" | "Assessment" | "Quiz"; }> = ({ type, testType }) => {
    ...
    const Redirect = (testStep: string) => Router.push(`/b2c-phase-3/b2c-teacher/teacher-flow/create-${label}?step=${testStep}`);
    return (
...
const TestListItem: React.FC<{ test: Test, type: TestType, testType?: "BW Test" | "Assessment" | "Quiz"; }> = ({ test, type, testType }) => {
    ...
    const handleClick = () => {
        if (type === "completed") {
            Router.push(`/b2c-phase-3/b2c-teacher/teacher-flow/${label}student-list`);
        }
    }
