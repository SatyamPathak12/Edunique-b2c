...
const NavItem = ({ icon: Icon, label, href, active = false }: { icon: React.ElementType; label: string; href: string; active?: boolean; }) => {
  const isMyLearnings = label === "My Learnings";
  const textColor = isMyLearnings || active ? "text-[#FFCC00]" : "text-white";
  const hoverClass = isMyLearnings ? "" : "hover:bg-[#3366FF]/70";
  ...
  return (
    <Link href={href} className={`flex items-center text-nowrap gap-2 sm:px-2 sm:py-2 text-xs sm:text-sm font-semibold rounded-full transition-colors ${textColor} ${hoverClass}`}>
      <Icon className={`w-5 h-3 sm:w-5 sm:h-5 ${isMyLearnings ? "text-[#FFCC00]" : ""}`} />
      {label}
    </Link>
  );
};
        ...
        <div className="mx-auto h-20 flex justify-between items-center max-w-screen-2xl">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="hidden lg:flex items-center gap-2">
              ...
              <nav className="flex md:space-x-6 items-center bg-[#E3F2FD26] rounded-full sm:px-4 md:px-6 sm:py-3">
                {navItems.map((item) => (
          ...
          <div className="flex items-center space-x-2 md:space-x-4">
            {isAskme && (
              <button className="flex items-center gap-2 px-3 py-2.5 bg-[#E3F2FD26] text-white text-sm font-medium rounded-full hover:bg-white/30 transition-colors">
